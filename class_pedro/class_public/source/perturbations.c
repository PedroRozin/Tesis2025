    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          /*acá agrego el store de los datos de delta_prime*/
          class_store_double(dataptr,tk[ppt->index_tp_delta_prime_cdm],ppt->has_source_delta_prime_cdm,storeidx); 
          class_store_double(dataptr,tk[ppt->index_tp_delta_prime_b],ppt->has_source_delta_prime_b,storeidx);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      /*acá agrego los nombres de las columnas*/
      class_store_columntitle(titles,"d_prime_cdm",ppt->has_source_delta_cdm);
      class_store_columntitle(titles,"d_prime_b",ppt->has_source_delta_b);
      if (pba->has_ncdm == _TRUE_) {
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++) {
...
int perturbations_indices(
  ...
  ppt->has_source_delta_prime_cdm = _TRUE_;
  ppt->has_source_delta_prime_b = _TRUE_;
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_cdm == _TRUE_)
          ppt->has_source_delta_cdm = _TRUE_; /*acá podría poner delta_prime, pero ya la definí TRUE antes*/
      ...
      // class_define_index(ppt->index_tp_delta_prime_b,    ppt->has_source_delta_b,   index_type,1); // NEW: baryon density derivative
      // class_define_index(ppt->index_tp_delta_prime_cdm,  ppt->has_source_delta_cdm, index_type,1); // NEW: cdm density derivative
      class_define_index(ppt->index_tp_delta_prime_cdm, _TRUE_, index_type, 1); /*acá agrego forzado*/
      // ppt->has_source_delta_prime_cdm = _TRUE_;
      class_define_index(ppt->index_tp_delta_prime_b, _TRUE_, index_type, 1);
      // ppt->has_source_delta_prime_b = _TRUE_;
...
/** matter_perturbations en este metodo se resuelven las ecuaciones diferenciales, ver mas abajo */
...
/** matter_perturbations acá se integran las perturbaciones con el metodo generic_evolver */
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      // acá agrego delta_prime_b
      class_store_columntitle(ppt->scalar_titles,"delta_prime_b",ppt->has_source_delta_prime_b);
      ...
      // acá pongo delta_prime_cdm
      class_store_columntitle(ppt->scalar_titles,"delta_prime_cdm",pba->has_cdm);
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double velocity_tot;
  double s2_squared;
  //acá add delta primes:
  double delta_prime_cdm;
  double delta_prime_b;
  double delta_prime_tot;
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_m == _TRUE_) { /** esto es delta_matter? */
    ...
    if (ppt->has_source_delta_b == _TRUE_) {
      _set_source_(ppt->index_tp_delta_b) = y[ppw->pv->index_pt_delta_b]; /*acá se almacena delta_b en ppt-> index_tp_delta_b para algún k y tau*/
        // + 3.*a_prime_over_a*theta_over_k2; // N-body gauge correction
    }

    /* delta_cdm */
    if (ppt->has_source_delta_cdm == _TRUE_) {
      _set_source_(ppt->index_tp_delta_cdm) = y[ppw->pv->index_pt_delta_cdm]; /*acá se almacena delta_cdm en ppt-> index_tp_delta_cdm para algún k y tau*/
        // + 3.*a_prime_over_a*theta_over_k2; // N-body gauge correction
    }
    ...
    if (ppt->has_source_delta_b == _TRUE_) {
      _set_source_(ppt->index_tp_delta_prime_b) = dy[ppw->pv->index_pt_delta_b];
      }
...
int perturbations_print_variables(double tau,
  ...
  // acá delta_primes en perturbations_print_variables
  double delta_prime_cdm=0., delta_prime_b=0.;
  ...
  if (_scalars_) {
    ...
    // ACÁ AGREGO LAS DERIVADAS QUE VAN A ESTAR EN EL '...perturbations_k0's.dat'
    delta_b = y[ppw->pv->index_pt_delta_b];
    // acá agrego el delta_prime_b
    delta_prime_b = dy[ppw->pv->index_pt_delta_b];
    theta_b = y[ppw->pv->index_pt_theta_b];
    ...
    if (pba->has_cdm == _TRUE_) {
      // ACÁ AGREGO LAS DERIVADAS PARA DELTA CDM
      delta_cdm = y[ppw->pv->index_pt_delta_cdm];
      // acá agrego el delta_prime_cdm
      delta_prime_cdm = dy[ppw->pv->index_pt_delta_cdm];
      if (ppt->gauge == synchronous) {
        theta_cdm = 0.;
      }
      else {
    ...
    // acá agrego el delta_prime_b. este store es para el de perturbations_k0_s.dat
    class_store_double(dataptr, delta_prime_b, _TRUE_, storeidx);
...
/**matter_perturbations:ecuaciones para bariones para ambos gauges */
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->gauge == newtonian) {
        ...
        /*DEBUG*/
        /*create a file with delta, delta_prime, a and k*/
        FILE * fdelta_cdm;
        fdelta_cdm = fopen("delta_prime_cdm.txt","a");
        fprintf(fdelta_cdm,"%e %e %e %e %e %e %e\n",y[pv->index_pt_delta_cdm],
          dy[pv->index_pt_delta_cdm], y[pv->index_pt_delta_b], dy[pv->index_pt_delta_b],a,k, a_prime_over_a);
        fclose(fdelta_cdm);

        dy[pv->index_pt_theta_cdm] = - a_prime_over_a*y[pv->index_pt_theta_cdm] + metric_euler; /* cdm velocity */
      }
/** matter_perturbations: cold dark matter */
    ...
    /**ACÁ ESTÁN DEFINIDOS LOS GAUGES */
    if (ppt->gauge == synchronous) {

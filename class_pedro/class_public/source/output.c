...
int output_pk(
  ...
  double * delta_prime_cdm;  /* array delta_prime_cdm[index_k] */
  double * delta_prime_b;    /* array delta_prime_b[index_k] */
  ...
  int index_ic = 0; /* for simplicity, we only output for the first IC */
  ...
  /* ACÁ GUARDO LOS VALORES DE DELTA_PRIME - allocate arrays for delta_prime values */
  class_alloc(delta_prime_b,
              pfo->k_size*sizeof(double),
              pop->error_message);
  
  class_alloc(delta_prime_cdm,
              pfo->k_size*sizeof(double),
              pop->error_message);
  ...
  if ((pk_output == pk_linear) || (pk_output == pk_nonlinear)) {
    ...
    for (index_pk=0; index_pk<pfo->pk_size; index_pk++) {
      ...
      for (index_z = 0; index_z < pop->z_pk_num; index_z++) {
        ...
        for (index_k=0; index_k<pfo->k_size; index_k++) {
          ...
          /* We need to retrieve delta_prime_cdm and delta_prime_b values */
          /* Get tau corresponding to this redshift */
          ...
          /* Allocate and get transfer functions at this time and k */
          class_alloc(tkfull, 
          ...
          /* acá obtengo delta_prime_cdm - esta es la que usamos en perturbations.*/
          class_call(perturbations_sources_at_tau(ppt,
                                          index_md,
                                          index_ic,
                                          ppt->index_tp_delta_prime_cdm,
                                          tau,
                                          pvecsources),
                   ppt->error_message,
                   pop->error_message);
          delta_prime_cdm[index_k] = pvecsources[index_k];
          
          /* acá get delta_prime_b */
          class_call(perturbations_sources_at_tau(ppt,
                                          index_md,
                                          index_ic,
                                          ppt->index_tp_delta_prime_b,
                                          tau,
                                          pvecsources),
                   ppt->error_message,
                   pop->error_message);
          delta_prime_b[index_k] = pvecsources[index_k];
          
          free(pvecsources);
          free(tkfull);
          
          /*acá es para power spectrum, pero seguro haya que volarlo*/
          class_call(output_one_line_of_pk(out_pk,
                                exp(pfo->ln_k[index_k])/pba->h,
                                exp(ln_pk[index_k])*pow(pba->h,3),
                                delta_prime_cdm[index_k],
                                delta_prime_b[index_k]
                                ),
            pop->error_message,
            pop->error_message);  
    ...
    for (index_z = 0; index_z < pop->z_pk_num; index_z++) {
      ...
      for (index_k=0; index_k<pfo->k_size; index_k++) {
        /*idem para power spectrum*/  
        class_call(output_one_line_of_pk(out_pk,
                                         exp(pfo->ln_k[index_k])/pba->h,
                                         exp(ln_pk[index_k])*pow(pba->h,3),
                                         0.0, /* Placeholder for delta_prime_cdm - not available for nowiggle spectrum */
                                         0.0  /* Placeholder for delta_prime_b - not available for nowiggle spectrum */
                                         ),
                   pop->error_message,
                   pop->error_message);
  ...
  /** - deal with analytic nowiggle power spectrum at z=0 */
    ...
    for (index_k=0; index_k<pfo->k_size; index_k++) {
      ...
      class_call(output_one_line_of_pk(out_pk,
                                       ...
                                       0.0, /* Placeholder for delta_prime_cdm - not available for analytic nowiggle spectrum */
                                       0.0  /* Placeholder for delta_prime_b - not available for analytic nowiggle spectrum */
                                       ),
                 pop->error_message,
                 pop->error_message);
    }
...
int output_open_pk_file(
  ...
  if (pop->write_header == _TRUE_) {
    ...
    // class_fprintf_columntitle(*pkfile,"delta_prime_cdm",_TRUE_,colnum);
    // class_fprintf_columntitle(*pkfile,"delta_prime_b",_TRUE_,colnum);
...
int output_one_line_of_pk(
                          ...
                          double one_k,
                          double one_pk,
                          double one_delta_prime_cdm,
                          double one_delta_prime_b
                          ) {

  fprintf(pkfile," ");
  class_fprintf_double(pkfile,one_k,_TRUE_);
  class_fprintf_double(pkfile,one_pk,_TRUE_);
  // class_fprintf_double(pkfile,one_delta_prime_cdm,_TRUE_);
  // class_fprintf_double(pkfile,one_delta_prime_b,_TRUE_);
  fprintf(pkfile,"\n");
